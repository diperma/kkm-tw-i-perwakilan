...
import {
    Search, Filter, CheckCircle2, XCircle, ExternalLink, FolderOpen,
    PieChart, Clock, LayoutGrid, Table2, BarChart3, Camera, Activity,
    FilePlus, FileEdit, Trash2, RefreshCw, LogOut, Database, Loader2,
    AlertCircle, Wifi, WifiOff
} from 'lucide-react';

export default function DashboardPage() {
    const { user, logout, driveToken } = useAuth();
    const { data: perwakilanData, loading: dataLoading, error: dataError, usingFallback, refetch: refetchDriveData } = usePerwakilan();
    const { logs: activityLogs, loading: logsLoading, usingFallback: logsFallback } = useActivityLogs();
    ...
    const handleSeed = async () => {
        ...
        try {
            ...
            setSeedMessage(created ? 'Data berhasil di-seed ke Firestore!' : 'Data sudah ada di Firestore.');
        } catch (err) {
    ...
    }, [perwakilanData, searchTerm, filterStatus]);

    // Statistics
    const stats = useMemo(() => {
        const total = perwakilanData.length;
        const submitted = perwakilanData.filter(item => item.status === 'submitted').length;
    ...
    const regionGroups = useMemo(() => {
        ...
        perwakilanData.forEach(item => {
            ...
            if ((idNum >= 1 && idNum <= 8) || idNum === 28 || idNum === 29) region = 'Sumatera';
            else if ((idNum >= 9 && idNum <= 13) || idNum === 30) region = 'Jawa';
            else if ((idNum >= 14 && idNum <= 17) || idNum === 34) region = 'Kalimantan';
            else if ((idNum >= 18 && idNum <= 21) || idNum === 31 || idNum === 32) region = 'Sulawesi';
        ...
        return (
            <div className="min-h-screen bg-slate-50 flex items-center justify-center">
                <div className="text-center animate-fade-in">
                    ...
                    <p className="text-slate-500 font-medium">Memuat data perwakilan...</p>
                </div>
            </div>
        );
    }
                    ...
                    <div className="flex justify-between items-center h-16">
                        <div className="flex items-center space-x-3">
                            ...
                            <div>
                                ...
                                <p className="text-xs text-slate-500 font-medium">Dashboard Kertas Kerja Perwakilan</p>
                            </div>
                        </div>

                        <div className="flex items-center space-x-4">
                            {/* Connection status */}
                            <div className={`hidden sm:flex items-center space-x-1.5 text-xs font-medium px-2.5 py-1 rounded-full ${usingFallback
                            ...
                            <div className="hidden md:flex items-center space-x-2 text-sm text-slate-500 bg-slate-100 px-3 py-1.5 rounded-full font-medium">
                                ...
                                <span>Update: Hari ini, 09:00 WIB</span>
                            </div>
            ...
            <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                ...
                {usingFallback && (
                    <div className="mb-6 p-4 bg-amber-50 border border-amber-200 rounded-xl flex items-start space-x-3 animate-fade-in">
                        ...
                        <div className="flex-1">
                            <p className="text-amber-800 text-sm font-semibold">Mode Data Demo (Drive API Gagal)</p>
                            <p className="text-amber-700 text-xs mt-0.5">
                                {dataError ? dataError.message : 'Silakan login dengan akun Google Workspace yang memiliki akses ke folder terkait.'}
                            </p>
                        </div>
                    </div>
                )}
                ...
                <div className="mb-8 animate-fade-in">
                    <div className="flex items-end justify-between mb-2">
                        <h2 className="text-lg font-bold text-slate-800">Progress Nasional</h2>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                    <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6 flex items-center space-x-4 animate-slide-up" style={{ animationDelay: '0ms' }}>
                        <div className="bg-blue-50 p-3 rounded-full">
                            <PieChart className="h-8 w-8 text-blue-600" />
                        </div>
                        <div>
                            <p className="text-sm font-medium text-slate-500">Total Perwakilan</p>
                            <p className="text-3xl font-bold text-slate-900">{stats.total}</p>
                        </div>
                    </div>

                    <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6 flex items-center space-x-4 relative overflow-hidden animate-slide-up" style={{ animationDelay: '100ms' }}>
                    ...
                    <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6 flex items-center space-x-4 relative overflow-hidden animate-slide-up" style={{ animationDelay: '200ms' }}>
                ...
                <div className="flex flex-col md:flex-row justify-between items-center bg-white p-4 rounded-xl shadow-sm border border-slate-200 mb-6 gap-4">
                    <div className="flex flex-col sm:flex-row items-center space-y-4 sm:space-y-0 sm:space-x-4 w-full md:w-auto">
                        ...
                        <div className="flex items-center space-x-2 w-full sm:w-auto">
                            ...
                            <select
                                className="block w-full sm:w-40 pl-3 pr-8 py-2 text-base border-slate-300 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm rounded-lg bg-slate-50 border"
                                value={filterStatus}
                ...
                {viewMode === 'activity' ? (
                    <div className="bg-white p-6 md:p-8 rounded-2xl shadow-sm border border-slate-200 animate-fade-in">
                        <div className="mb-6 border-b border-slate-200 pb-4">
                            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-2 gap-2">
                                ...
                                <span className={`flex items-center text-xs sm:text-sm px-3 py-1.5 rounded-full font-bold border ${logsFallback
                                    ? 'text-amber-700 bg-amber-50 border-amber-200'
                                    : 'text-green-700 bg-green-50 border-green-200'
                                    }`}>
                                    <RefreshCw className={`w-4 h-4 mr-2 ${logsFallback ? '' : 'animate-[spin_3s_linear_infinite]'}`} />
                        ...
                        <div className="relative border-l-2 border-slate-200 ml-4 md:ml-6 space-y-8 pb-4">
                            {activityLogs.map((log) => {
                                ...
                                let actionText = 'mengunggah file baru';

                                if (log.action === 'rename') {
                                    Icon = FileEdit; iconBg = 'bg-amber-100'; iconColor = 'text-amber-600'; actionText = 'mengubah nama file';
                                } else if (log.action === 'delete') {
                                    Icon = Trash2; iconBg = 'bg-red-100'; iconColor = 'text-red-600'; actionText = 'menghapus file';
                                } else if (log.action === 'create_folder') {
                                    Icon = FolderOpen; iconBg = 'bg-indigo-100'; iconColor = 'text-indigo-600'; actionText = 'membuat folder baru';
                                }

                                return (
                                    <div key={log.id} className="relative pl-8 md:pl-10">
                                        <div className={`absolute -left-[17px] top-1 w-8 h-8 rounded-full ${iconBg} border-4 border-white flex items-center justify-center shadow-sm`}>
                                        ...
                                        <div className="bg-slate-50 rounded-lg p-4 border border-slate-100 shadow-sm hover:shadow-md transition-shadow">
                                            <div className="flex flex-col sm:flex-row justify-between items-start mb-2 sm:mb-1 gap-2 sm:gap-0">
                                                <p className="text-sm font-medium text-slate-800">
                                                    <span className="font-bold">{log.user}</span> ({log.pwId}) {actionText}
                                                </p>
                                                <span className="text-xs text-slate-500 whitespace-nowrap bg-white px-2 py-1 rounded-md border border-slate-200">
                                                    {log.time}
                                                </span>
                                            </div>
                                            <p className="text-sm font-bold text-slate-900 bg-white px-3 py-2 rounded border border-slate-200 mt-2 inline-block">
                                                {log.file}
                                            </p>
                                        </div>
                                    </div>
                    ...
                    <div className="bg-white p-6 md:p-8 rounded-2xl shadow-sm border border-slate-200 animate-fade-in">
                        <div className="mb-6 border-b border-slate-200 pb-4 flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
                            <div>
                                <h2 className="text-2xl font-black text-slate-900">Laporan Monitoring Kertas Kerja</h2>
                                <p className="text-slate-500 mt-1">Status per Hari Ini, 09:00 WIB</p>
                            </div>
                            <div className="text-left md:text-right bg-blue-50 p-3 rounded-lg border border-blue-100">
                                <p className="text-xs font-bold text-blue-600 uppercase tracking-wider mb-1">Total Progress Nasional</p>
                                <p className="text-3xl font-black text-blue-700 leading-none">{stats.percentage}%</p>
                            </div>
                        </div>
                        ...
                        <div className="mb-10 rounded-xl border border-slate-200 p-4 md:p-6 bg-slate-50/50 overflow-x-auto">
                            ...
                            <div className="flex items-end justify-between h-56 mt-4 space-x-2 sm:space-x-4 min-w-[500px]">
                                {regionGroups.map(region => (
                                    <div key={region.name} className="flex flex-col items-center flex-1 h-full justify-end group">
                                        <div className="w-full flex justify-center space-x-1 sm:space-x-2 items-end h-40">
                                            <div className="w-1/2 max-w-[40px] bg-green-500 rounded-t-md relative flex flex-col justify-end items-center transition-all duration-500"
                                                style={{ height: `${(region.submitted / maxRegionTotal) * 100}%`, minHeight: region.submitted > 0 ? '4px' : '0' }}>
                                                <span className="text-xs sm:text-sm font-bold text-green-700 -mt-6">{region.submitted}</span>
                                            </div>
                                            <div className="w-1/2 max-w-[40px] bg-red-400 rounded-t-md relative flex flex-col justify-end items-center transition-all duration-500"
                                                style={{ height: `${(region.missing / maxRegionTotal) * 100}%`, minHeight: region.missing > 0 ? '4px' : '0' }}>
                                                <span className="text-xs sm:text-sm font-bold text-red-700 -mt-6">{region.missing}</span>
                                            </div>
                                        </div>
                                        <span className="text-xs sm:text-sm font-bold text-slate-700 mt-4 text-center leading-tight h-10 flex items-center">{region.name}</span>
                                    </div>
                                ))}
                            </div>

                            <div className="flex justify-center space-x-8 mt-4 pt-4 border-t border-slate-200 min-w-[500px]">
                                <div className="flex items-center"><div className="w-4 h-4 bg-green-500 rounded mr-2 shadow-sm"></div><span className="text-sm font-medium text-slate-700">Telah Submit</span></div>
                                <div className="flex items-center"><div className="w-4 h-4 bg-red-400 rounded mr-2 shadow-sm"></div><span className="text-sm font-medium text-slate-700">Belum Submit</span></div>
                            </div>
                        </div>
                            ...
                            <div className="overflow-x-auto rounded-xl border border-slate-200 shadow-sm">
                                <table className="min-w-full divide-y divide-slate-200">
                                    <thead className="bg-slate-100">
                                        <tr>
                                            <th scope="col" className="px-6 py-4 text-left text-xs font-bold text-slate-600 uppercase tracking-wider">ID</th>
                                            <th scope="col" className="px-6 py-4 text-left text-xs font-bold text-slate-600 uppercase tracking-wider">Nama Perwakilan</th>
                                            <th scope="col" className="px-6 py-4 text-left text-xs font-bold text-slate-600 uppercase tracking-wider">Status</th>
                                            <th scope="col" className="px-6 py-4 text-left text-xs font-bold text-slate-600 uppercase tracking-wider">Tgl Update</th>
                                        </tr>
                                    </thead>
                                    <tbody className="bg-white divide-y divide-slate-200">
                                        {filteredData.map((item) => (
                                            <tr key={item.id} className="hover:bg-slate-50 transition-colors">
                                                <td className="px-6 py-3 whitespace-nowrap text-sm font-bold text-slate-900">{item.id}</td>
                                                ...
                                                <td className="px-6 py-3 whitespace-nowrap">
                                                    {item.status === 'submitted' ? (
                                                        <span className="inline-flex items-center px-3 py-1 rounded-full text-xs font-bold bg-green-100 text-green-800 border border-green-200">
                                                            <CheckCircle2 className="w-4 h-4 mr-1" /> Telah Submit
                                                        </span>
                                                    ) : (
                            ...
                            >
                                ...
                                <div className="mt-5 pt-4 border-t border-slate-100">
                                    {item.status === 'submitted' && item.driveFileId ? (
                                        ...
                                        <button className="w-full flex items-center justify-center space-x-2 px-4 py-2 text-sm font-medium rounded-lg transition-colors bg-slate-50 text-slate-500 hover:bg-slate-100 hover:text-slate-700 disabled:opacity-50" disabled>
