...
import { fetchDriveContents, checkIfFolderHasFiles } from '../services/driveService';
import { PERWAKILAN_DATA, getFallbackPerwakilan } from '../services/firestoreService';
...
export function usePerwakilan() {
    ...
    const fetchData = async () => {
        ...
        // Jika user adalah guest atau token tidak ada, gunakan fallback
        if (user?.uid === 'guest-123' || !driveToken) {
            setData(getFallbackPerwakilan());
        ...
        try {
            ...
            const mappedPromises = PERWAKILAN_DATA.map(async pw => {
                ...
                if (matchedFile) {
                    ...
                    const hasFiles = await checkIfFolderHasFiles(driveToken, matchedFile.id);
                    if (hasFiles) {
                        return {
                            ...pw,
                            status: 'submitted',
                            lastUpdated: new Date(matchedFile.modifiedTime).toLocaleDateString('id-ID'),
